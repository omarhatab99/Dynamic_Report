...
@Component({
  selector: 'app-nested-pyramid-grid-wrapper',
  templateUrl: './nested-pyramid-grid-wrapper.component.html',
  styleUrls: ['./nested-pyramid-grid-wrapper.component.css']
})
export class NestedPyramidGridWrapperComponent implements OnInit, AfterViewInit {

  @ViewChild("handsontableNestedContainer") handsontableNestedContainer!: ElementRef;
  @ViewChild("targetSection") targetSection!: ElementRef;
  @ViewChild("exportationPreview") exportationPreview!: ExportationComponent;
  @ViewChild("testing") testing!: ElementRef;
  //statics
  static nestedTableStatic: NestedPyramidGridWrapperComponent[] = [];
  ...
  @Input() rowHeaderWidth: number = 50;
  ...
  sourceDataObject: any[] = [
    ...
    { isParent: false, isExpanded: false, الموازنه: "القسم الاول: الحكومه- الجهاز الادارى - الادارة المحليه - الخدمات المحليه", القيمه: '10574', children: [] },
    { isParent: false, isExpanded: false, الموازنه: "القسم الثالث: الوحدات الاقتصاديه غير المعامله بالقاون رقم 903", القيمه: '5145', children: [] },
    { isParent: false, isExpanded: false, الموازنه: "القسم الرابع: موازنات خاصه ", القيمه: '1', children: [] },
    {
      isParent: true, isExpanded: false, "الموازنه": "القسم الثانى: الهيئات الاقتصاديه", القيمه: '552', children: [
        {
          isParent: true, isExpanded: false, "التبعيه": "الهيئات الاقتصاديه", القيمه: '552', children: [
  ...
  ngOnInit(): void {
    ...
    NestedPyramidGridWrapperComponent.nestedTableStatic.push(this);
    this.printSettings.screenMode = "landscape";
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      this.handleAllNestedInIntializationOfHandsontable();
    }, 50);
  ...
  createDynamicNestedHandsontable(row: number, rowElement: HTMLElement, data: any) {
    ...
    handsontableContainer.className = "nested-handsontable inner-handsontable";
    ...
    const componentFactory = this.componentFactoryResolver.resolveComponentFactory(NestedPyramidGridWrapperComponent);
    const componentRef = this.viewContainerRef.createComponent(componentFactory);

    //reassign component configurations.
    componentRef.instance.showHandsontableConfiguration = false;
    componentRef.instance.rowHeaderWidth = this.rowHeaderWidth * 1.5;
  ...
  handleAllNestedInIntializationOfHandsontable() {
    ...
    if (this.handsontableNestedContainer.nativeElement
      .querySelectorAll(".collapsed-row .expande-btn").length > 0) {
      this.handsontableNestedContainer.nativeElement
        .querySelectorAll(".collapsed-row .expande-btn").forEach((btn: HTMLElement) => {
          btn.click();
        });
  ...
  renderAllHandsontable() {
    ...
    for (let index = 0; index < NestedPyramidGridWrapperComponent.nestedTableStatic.length; index++) {

      setTimeout(() => {
        NestedPyramidGridWrapperComponent.nestedTableStatic[index]?.handsontableSettings?.hotInstance.refreshDimensions();
        NestedPyramidGridWrapperComponent.nestedTableStatic[index]?.handsontableSettings?.hotInstance.render();
      }, 0)

    }

    for (let index = NestedPyramidGridWrapperComponent.nestedTableStatic.length - 1; index >= 0; index--) {
  ...
  ngOnDestroy(): void {
    NestedPyramidGridWrapperComponent.nestedTableStatic = [];
  }
